    ...
    <div className="rounded-2xl border border-green-100 bg-white p-6 shadow-sm shadow-green-900/5">
      <div className="mb-5 flex items-start justify-between">
        <div>
          ...
          {goalPct !== null && !goalReached && <p className="mt-0.5 text-sm text-green-600">{Math.round(goalPct)}% to goal</p>}
        ...
        <div className="mb-2 flex items-end justify-between">
          <div>
            <p className="font-mono text-4xl font-semibold tabular-nums tracking-tight text-green-950">
              {entriesQuery.isLoading ? <span className="inline-block h-9 w-32 animate-pulse rounded-lg bg-green-100" /> : fmt(totalContributed)}
            </p>
            <p className="mt-0.5 text-sm text-green-600">
              {goalTargetNum !== null ? `of ${fmt(goalTargetNum)} · ${goal!.name}` : "total contributed · no goal set"}
            </p>
          </div>
          {monthsToGoal !== null && <p className="mb-0.5 text-sm text-green-500">~{monthsToGoal} mo to goal</p>}
        </div>
        {goalTargetNum !== null && (
          <div className="h-2 w-full overflow-hidden rounded-full bg-blue-100">
            <div className={`h-full rounded-full transition-all duration-500 ${goalReached ? "bg-blue-300" : "bg-blue-400"}`} style={{ width: `${goalPct ?? 0}%` }} />
          </div>
        )}
      </div>
      ...
      {!showAddForm ? (
        ...
        ><Plus size={13} /> add contribution</button>
      ) : (
        <form onSubmit={handleAddSubmit} className="mb-5 space-y-3 rounded-xl border border-green-200 bg-green-50 p-4">
          ...
          <div className="flex gap-2">
            ...
            <button type="submit" disabled={createEntry.isPending || !amount}
              className="shrink-0 rounded-full bg-blue-500 px-4 py-2 text-sm font-medium text-white transition hover:bg-blue-400 disabled:cursor-not-allowed disabled:bg-green-200 disabled:text-green-400"
      ...
      {!entriesQuery.isLoading && allEntries.length === 0 && (
        <div className="py-4 text-center">
          <p className="text-sm text-green-400">no contributions yet — start investing!</p>
        </div>
      )}
                    ...
                    {(txs ?? []).map((tx) => (
                      <li key={tx.id} className="group flex items-center justify-between py-2">
                        ...
                        <div className="flex items-center gap-3">
                          <p className="font-mono text-base tabular-nums text-blue-500">+{fmtFull(Number(tx.amount))}</p>
