...
function SavingCategoryCard({
  ...
  const goalPct =
    goalTargetNum !== null
      ? Math.min(100, (totalSaved / goalTargetNum) * 100)
      : null;
  const remainingToGoal =
    goalTargetNum !== null ? Math.max(0, goalTargetNum - totalSaved) : null;
    ...
    <div className="rounded-2xl border border-white/[0.06] bg-[#111111] p-6">
      ...
      <div className="mb-5 flex items-start justify-between">
        <div>
          ...
          {goalReached && (
            <p className="mt-0.5 text-xs text-emerald-400">goal reached 🎉</p>
          )}
        </div>
          ...
          <div className="h-1.5 w-full overflow-hidden rounded-full bg-white/[0.05]">
            <div
              className={`h-full rounded-full transition-all duration-500 ${
                goalReached ? "bg-emerald-300" : "bg-emerald-400"
              }`}
      ...
      <div className="mb-5 rounded-xl bg-white/[0.03] px-4 py-3">
        <div className="mb-1.5 flex items-center justify-between">
          ...
          <p className="font-mono text-xs tabular-nums text-neutral-400">
            {fmt(thisMonthTotal)}
            {monthlyAllocation > 0 && (
              <span className="text-neutral-600"> of {fmt(monthlyAllocation)}</span>
            )}
          </p>
        </div>
        {monthlyAllocation > 0 ? (
          <div className="h-1 w-full overflow-hidden rounded-full bg-white/[0.05]">
            <div
              className="h-full rounded-full bg-emerald-400 opacity-50 transition-all duration-500"
              style={{ width: `${monthlyPct}%` }}
            />
          </div>
        ) : (
          <p className="text-xs text-neutral-700">no monthly allocation — set one in the budget split</p>
        )}
      </div>
        ...
        >
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="shrink-0 rounded-full bg-emerald-500 px-4 py-2 text-xs font-medium text-white transition hover:bg-emerald-400 disabled:cursor-not-allowed disabled:bg-neutral-800 disabled:text-neutral-500"
            >
    ...
    <main className="mx-auto max-w-2xl px-6 py-10">
      ...
      {!isLoading && savingCategories.length === 0 && (
        <div className="py-16 text-center">
          <p className="text-neutral-500">no savings categories set up.</p>
          <p className="mt-1 text-xs text-neutral-700">
            add a savings category during onboarding or in your settings.
          </p>
          <Link
            href="/home"
